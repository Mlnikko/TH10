...
public static class PresentationBridge
{
    ...
    static readonly IPresentationUpdater[] _updaters = new IPresentationUpdater[EntityManager.MAX_ENTITIES];
    static int _nextId = 1;

    /// <summary>
    /// 注册一个 GameObject，并绑定更新器
    /// </summary>
    public static int Register(GameObject go, IPresentationUpdater updater)
    {
    ...
    /// 在 LateUpdate 中调用
    /// </summary>
    public static void UpdateAllPresentations(in EntityManager em)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            _updaters[id]?.UpdatePresentation(em, entity);
        }
    }
}
...
{
    void UpdatePresentation(in EntityManager em, Entity entity);
}